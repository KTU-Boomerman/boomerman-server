    ...
    public class PulseExplosion : IExplosionStrategy
    {
        public Explosions GetExplosions(Position position, TimeSpan delay)
        {
            ...
            for (int i = 1; i < 4; i++)
            {
                ...
                explosions.AddExplosion(new Explosion(new Position(position.X + tile.X, position.Y), newDelay));

                // add explosion to the top
                explosions.AddExplosion(new Explosion(new Position(position.X, position.Y + tile.Y), newDelay));
